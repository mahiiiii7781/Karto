...
export default function AdminVendorCategoriesScreen({ route, navigation }: any) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(false);
    setRefreshing(false);
  }, [selectedVendorId, routeRestaurantId]);
  ...
  const filtered = useMemo(() => {
    ...
    return categories.filter((c) => {
      const vendorMatch = selectedVendorId ? c.restaurantId === selectedVendorId : true;
      const textMatch =
        !q ||
      ...
      return vendorMatch && textMatch;
    });
  }, [categories, search, selectedVendorId]);

  const stats = useMemo(() => {
    return {
      total: filtered.length,
      active: filtered.filter((c) => c.isActive !== false).length,
      inactive: filtered.filter((c) => c.isActive === false).length,
      subCategories: filtered.reduce((sum, c) => sum + Number(c.subCategories?.length || 0), 0),
      items: filtered.reduce((sum, c) => sum + Number(c.menuItems?.length || 0), 0),
    };
  }, [filtered]);
  ...
  const requestCameraPermission = async () => {
    ...
    const result = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.CAMERA,
      {
        ...
        message: "Karto needs camera access to capture category image.",
  ...
  const pickImage = async (fromCamera = false) => {
    try {
      ...
      updateForm("image", {
        ...
        fileName: asset.fileName || `vendor-category-${Date.now()}.jpg`,
  ...
  const save = async () => {
    ...
    if (!form.name.trim()) {
      showMsg("warning", "Required", "Category name required hai.");
      return;
    }
    ...
    const res = editing
      ? await adminService.updateVendorCategory(editing.id, payload)
      : await adminService.createVendorCategory(payload);

    setSaving(false);

    if (res.error) {
      showMsg("error", "Save Failed", res.error.message || "Vendor category save nahi hui.");
      return;
    }
    ...
    showMsg(
      ...
      editing ? "Vendor category update ho gayi." : "Vendor category create ho gayi.",
      "Done",
      () => {
        closeMsg();
        load();
  ...
  const askDelete = (item: any) => {
    showMsg(
      "warning",
      "Delete Category?",
      `${item.name || "This category"} delete karni hai? Linked subcategories/items hue to backend safe block karega.`,
      "Delete",
      () => deleteItem(item.id),
      "Cancel",
      closeMsg
  ...
  const deleteItem = async (id: string) => {
    ...
    const res = await adminService.deleteVendorCategory(id);
    ...
    if (res.error) {
      setMsg({
        ...
        type: "error",
        title: "Delete Failed",
        message: res.error.message || "Category delete nahi hui.",
        primaryText: "Okay",
      });
      return;
    }
    ...
    setMsg({
      ...
      message: "Vendor category delete/block ho gayi.",
      primaryText: "Done",
      onPrimary: () => {
        closeMsg();
        load();
  ...
  const toggleStatus = async (item: any) => {
    const res = await adminService.updateVendorCategory(item.id, {
      isActive: item.isActive === false,
    });
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading vendor categories...</Text>
      </View>
    );
  }
      ...
      <FlatList
          ...
          <>
            <View style={styles.header}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.subtitle}>Vendor ke internal menu categories manage karo</Text>
              </View>
            ...
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitleNoMargin}>Category List</Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            <Icon name="albums-outline" size={44} color={THEME.yellow} />
            <Text style={styles.emptyTitle}>No vendor categories found</Text>
            <Text style={styles.emptyText}>Create first category like Pizza, Burgers, Sweets.</Text>
        ...
        renderItem={({ item }) => (
          <CategoryCard
            ...
            onSubCategories={() =>
              navigation.navigate("AdminVendorSubCategories", {
                restaurantId: item.restaurantId,
                vendorCategoryId: item.id,
              })
            }
          />
        )}
...
function CategoryCard({ item, onEdit, onDelete, onToggle, onSubCategories }: any) {
  ...
  return (
    <View style={styles.card}>
      <View style={styles.cardTop}>
        {imageUrl ? (
          ...
          <View style={styles.thumbFallback}>
            <Icon name="albums-outline" size={27} color={THEME.yellow} />
          </View>
        )}
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.vendorText} numberOfLines={1}>
            {item.restaurant?.name || "Vendor"} • Subcats {item.subCategories?.length || 0} • Items {item.menuItems?.length || 0}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.actionRow}>
        ...
        <Action icon="layers-outline" text="Subcats" onPress={onSubCategories} outline />
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalBox}>
          ...
          <View style={styles.modalHeader}>
            <View style={{ flex: 1 }}>
              <Text style={styles.modalLabel}>VENDOR CATEGORY</Text>
              <Text style={styles.modalTitle}>{editing ? "Edit Category" : "Create Category"}</Text>
            </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <Input
              ...
              placeholder="Example: Wood fired pizza"
              multiline
            />
            ...
            >
              ...
              <Text style={[styles.statusToggleText, form.isActive && styles.statusToggleTextActive]}>
                {form.isActive ? "Active Category" : "Blocked Category"}
              </Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  miniStat: { width: "31.5%", backgroundColor: THEME.card, borderRadius: 20, padding: 14, borderWidth: 1, borderColor: THEME.border },
