...
export default function AdminMenuAddonsScreen({ route, navigation }: any) {
  ...
  const stats = useMemo(() => ({
    total: addons.length,
    active: addons.filter((a) => a.isActive !== false).length,
    inactive: addons.filter((a) => a.isActive === false).length,
    avgPrice: addons.length
      ? addons.reduce((s, a) => s + Number(a.price || 0), 0) / addons.length
      : 0,
  }), [addons]);
  ...
  const requestCameraPermission = async () => {
    ...
    const result = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.CAMERA,
      {
        ...
        message: "Karto needs camera access to capture addon image.",
  ...
  const pickImage = async (camera = false) => {
    try {
      ...
      updateForm("image", {
        ...
        fileName: asset.fileName || `addon-${Date.now()}.jpg`,
        name: asset.fileName || `addon-${Date.now()}.jpg`,
      });
    } catch (e: any) {
  ...
  const openEdit = (addon: any) => {
    setEditing(addon);
    setForm({
      menuItemId: addon.menuItemId || selectedItemId,
      title: addon.title || addon.name || "",
      name: addon.name || addon.title || "",
      price: String(addon.price || ""),
      image: null,
      existingImageUrl: addon.imageUrl || addon.image_url || "",
  ...
  const save = async () => {
    ...
    if (!form.title.trim() && !form.name.trim()) {
      showMsg("warning", "Required", "Addon name required hai.");
      return;
    }

    if (String(form.price).trim() === "" || Number.isNaN(Number(form.price))) {
      showMsg("warning", "Invalid Price", "Addon price valid number hona chahiye.");
      return;
    }
    ...
    const res = editing
      ? await adminService.updateMenuItemAddon(editing.id, payload)
      : await adminService.createMenuItemAddon(form.menuItemId, payload);

    setSaving(false);

    if (res.error) {
      showMsg("error", "Save Failed", res.error.message || "Addon save nahi hua.");
      return;
    }

    closeModal();
    showMsg("success", editing ? "Addon Updated" : "Addon Created", "Addon saved successfully.", "Done", () => {
      closeMsg();
      load();
    });
  };

  const askDelete = (addon: any) => {
    showMsg(
      "warning",
      "Delete Addon?",
      `${addon.title || addon.name || "This addon"} delete karna hai?`,
      "Delete",
  ...
  const deleteAddon = async (id: string) => {
    ...
    const res = await adminService.deleteMenuItemAddon(id);
    ...
    if (res.error) {
      setMsg({
        ...
        type: "error",
        title: "Delete Failed",
        message: res.error.message || "Addon delete nahi hua.",
        primaryText: "Okay",
      });
      return;
    }
  ...
  const toggleStatus = async (addon: any) => {
    const res = await adminService.updateMenuItemAddon(addon.id, {
      isActive: addon.isActive === false,
    });
      ...
      <FlatList
          ...
          <>
            <View style={styles.header}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.title}>Menu Addons</Text>
                <Text style={styles.subtitle}>Extra cheese, dip, toppings and paid add-ons</Text>
              </View>
            ...
            <View style={styles.heroCard}>
              <View>
                ...
                <Text style={styles.heroSub}>
                  {selectedItem?.restaurant?.name || "Vendor"} • {money(selectedItem?.price)}
                </Text>
              </View>

              <View style={styles.heroIcon}>
                <Icon name="add-circle-outline" size={34} color={THEME.yellow} />
              </View>
            </View>

            <View style={styles.statGrid}>
              <MiniStat label="Addons" value={stats.total} />
            ...
            <View style={styles.sectionHeader}>
              <Text style={styles.sectionTitleNoMargin}>Addon List</Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            <Icon name="add-circle-outline" size={44} color={THEME.yellow} />
            <Text style={styles.emptyTitle}>No addons found</Text>
            <Text style={styles.emptyText}>Create addons like Extra Cheese, Mayo, Dip.</Text>

            <TouchableOpacity style={styles.emptyAddBtn} onPress={openCreate}>
              <Icon name="add-circle-outline" size={21} color="#000" />
              <Text style={styles.emptyAddText}>Add Addon</Text>
            </TouchableOpacity>
          </View>
        }
        renderItem={({ item }) => (
...
function AddonCard({ item, onEdit, onDelete, onToggle }: any) {
      ...
      <View style={styles.cardTop}>
        {imageUrl ? (
          ...
          <View style={styles.thumbFallback}>
            <Icon name="add-circle-outline" size={27} color={THEME.yellow} />
          </View>
        )}
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.meta} numberOfLines={1}>
            {active ? "Active addon" : "Blocked addon"}
          </Text>
        </View>
      </View>
...
function AddonModal({ visible, editing, form, updateForm, saving, onCamera, onGallery, onClose, onSave }: any) {
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalBox}>
          ...
          <View style={styles.modalHeader}>
            <View style={{ flex: 1 }}>
              <Text style={styles.modalLabel}>MENU ADDON</Text>
              <Text style={styles.modalTitle}>{editing ? "Edit Addon" : "Create Addon"}</Text>
            </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <Text style={styles.inputLabel}>Addon Image</Text>
            ...
            >
              ...
              <Text style={[styles.statusToggleText, form.isActive && styles.statusToggleTextActive]}>
                {form.isActive ? "Active Addon" : "Blocked Addon"}
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={[styles.saveBtn, saving && { opacity: 0.7 }]} onPress={onSave} disabled={saving}>
              {saving ? <ActivityIndicator color="#000" /> : (
                <>
                  ...
                  <Text style={styles.saveText}>Save Addon</Text>
                </>
              )}
            </TouchableOpacity>
