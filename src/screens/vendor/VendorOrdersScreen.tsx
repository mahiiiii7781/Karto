...
export default function VendorOrdersScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    socket.on("vendor:newOrder", handleNewOrder);
    socket.on("new_order", handleNewOrder);
    socket.on("ORDER_CREATED", handleNewOrder);

    socket.on("ORDER_STATUS_UPDATED", handleOrderUpdate);
    socket.on("vendor:orderStatusUpdated", handleOrderUpdate);
    socket.on("orderStatusUpdated", handleOrderUpdate);
    socket.on("order-updated", handleOrderUpdate);
    socket.on("RIDER_ASSIGNED", handleOrderUpdate);
    socket.on("vendor:riderAssigned", handleOrderUpdate);

    socket.on("VENDOR_DASHBOARD_REFRESH", loadOrders);
    socket.on("vendor:dashboardRefresh", loadOrders);
  ...
  }, [accessToken, loadOrders, showToast, upsertOrder, user?.id, user?.role]);
  ...
  const updateStatus = async (
    ...
    setActionOrderId(null);
    showToast(status === "CANCELLED" ? "Order rejected" : "Order updated");
  };
  ...
  const assignRider = async (rider: RiderItem) => {
    ...
    const apiCall = selectedOrder.rider
      ? vendorService.reassignRider
      : vendorService.assignRider;

    const { data, error } = await apiCall(selectedOrder.id, rider.id);
    ...
    setSelectedOrder(null);
    setActionOrderId(null);
    showToast(selectedOrder.rider ? "Rider reassigned" : "Rider assigned");
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>
            Manage live orders, prep time and riders.
          </Text>
        </View>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => setPrepModal(false)}
      >
      ...
      <View style={styles.metaRow}>
        ...
        {!!order.rider && (
          <View style={styles.metaPill}>
            ...
            <Text style={styles.metaText}>
              {order.rider?.fullName || "Rider assigned"}
            </Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.itemsBox}>
        {items.slice(0, 3).map((i: any) => (
          <Text key={i.id} style={styles.itemLine}>
            {i.quantity}x {i.menuItem?.name || i.itemName || i.item_name || "Item"}
          </Text>
        ))}
  ...
  return (
    ...
    >
      {disabled ? (
        <ActivityIndicator
          size="small"
          color={filled || danger ? THEME.bg : THEME.green}
        />
      ) : (
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={styles.modalOverlay} onPress={onClose}>
        <Pressable style={styles.modalCard}>
          ...
          <Text style={styles.modalSub}>
            Choose an available rider for this ready order.
          </Text>
          ...
          {loading ? (
            ...
            <View style={styles.emptyRiderBox}>
              ...
              <Text style={styles.emptyText}>
                Try again after a few minutes or assign from admin panel.
              </Text>
            </View>
          ) : (
            <FlatList
              ...
              renderItem={({ item }) => {
                const name = item.fullName || item.full_name || "Rider";
